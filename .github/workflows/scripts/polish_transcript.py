...
if not os.path.exists(INP) or os.path.getsize(INP) == 0:
    raise SystemExit(0)
t0=time.time()
raw = open(INP, "r", "r", encoding="utf-8", errors="ignore").read()
...
def clean_text(s: str) -> str:
    ...
    return s
stats={"nodes":0,"emoji_dropped":0,"changed":0,"duration_sec":0.0,"total_words":0}
out = []
pos = 0
for m in TEXT_NODE.finditer(raw):
    ...
    stats["nodes"] += 1
    body_dec = html.unescape(body) # unescape for cleaning (original was escaped)
    plain = body_dec
    if is_emoji_only(plain):
        ...
        stats["total_words"] += n_words(cleaned)
        # re-escape only the critical chars (keep quotes as-is)
        new_body = html.escape(cleaned, quote=False)
